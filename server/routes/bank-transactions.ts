...
router.post('/', async (req: any, res: any) => {
  ...
  try {
    ...
    const [balanceResult] = await conn.query(
      `SELECT COALESCE(MAX(CASE WHEN bank_id = ? AND deposit > 0 THEN balance END), 0) as running_balance
       FROM bank_transactions 
       WHERE bank_id = ? AND shop_id = ?`,
      [bank_id, bank_id, req.shop.shop_id]
    );
...
router.get('/', async (req: any, res: any) => {
  ...
  try {
    ...
    const transactions = (rows as any[]).map((tx: any, index: number) => {
