...
router.post("/", async (req: AuthRequest, res) => {
  ...
  try {
    ...
    await conn.query(
      `UPDATE accounts SET balance = COALESCE(balance, 0) - ? WHERE id = ? AND shop_id = ?`,
      [numericAmount, paid_account_id, req.shop.shop_id]
    );
    await conn.query(
      `UPDATE accounts SET balance = COALESCE(balance, 0) + ? WHERE id = ? AND shop_id = ?`,
      [numericAmount, received_account_id, req.shop.shop_id]
    );
...
router.delete("/:id", async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const { id } = req.params;
    const [rows]: any = await conn.query(
      `SELECT paid_account_id, received_account_id, amount
       FROM transfers WHERE id = ? AND shop_id = ?`,
      [id, req.shop.shop_id]
    );
    if (!rows.length) {
      await conn.rollback();
    ...
    await conn.query(
      `UPDATE accounts SET balance = COALESCE(balance, 0) + ? WHERE id = ? AND shop_id = ?`,
      [amount, t.paid_account_id, req.shop.shop_id]
    );
    await conn.query(
      `UPDATE accounts SET balance = COALESCE(balance, 0) - ? WHERE id = ? AND shop_id = ?`,
      [amount, t.received_account_id, req.shop.shop_id]
    );
