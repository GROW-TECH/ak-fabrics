  ...
  try {
    ...
    await conn.query(
      `INSERT INTO purchases
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id, req.shop.shop_id, vendor_id, invoice_no, grandTotal,
    ...
    // Record initial payment if any paid amount
    if (paid > 0) {
      await conn.query(
...
router.get("/", async (req: AuthRequest, res) => {
  try {
    ...
    let query = `
      SELECT p.id, p.vendor_id, p.invoice_no, p.total_amount,
             p.payment_mode, p.paid_amount, p.balance_amount,
             p.payment_status, p.through_agent, p.notes,
             p.created_at, a.name AS vendor_name
      FROM purchases p
...
// ─────────────────────────────────────────────────────────────
//  GET /purchases/with-images  — Purchases with uploaded images
// ─────────────────────────────────────────────────────────────
router.get("/with-images", async (req: AuthRequest, res) => {
  try {
    ...
    const data = (rows as any[]).map((row) => ({
      ...
      imageUrl: `http://localhost:5000/uploads/purchase-invoices/${row.image_path}`,
...
router.get("/barcode/:invoiceNo", async (req: AuthRequest, res) => {
  try {
    ...
    const [purchaseRows]: any = await pool.query(
      `SELECT p.*, a.name AS vendor_name, a.address AS vendor_address,
...
// ─────────────────────────────────────────────────────────────
//  GET /purchases/:id/payments  — List payments for a purchase
// ─────────────────────────────────────────────────────────────
...
router.get("/:id/download", async (req: AuthRequest, res) => {
  try {
    ...
    const grandTotal = items.reduce((s: number, i: any) => s + Number(i.total), 0);
    doc.moveDown(2);
    doc.fontSize(12).font("Helvetica-Bold").text(`Grand Total: ₹${grandTotal.toLocaleString()}`, 0, y + 10, { align: "right" });
  ...
  try {
    ...
    await conn.query(
      `UPDATE purchases
       ...
       WHERE id = ? AND shop_id = ?`,
      [vendor_id, grandTotal, payment_mode, paid, balance, payment_status,
       through_agent, notes, id, req.shop.shop_id]
    );
    ...
    for (const item of items) {
      await conn.query(
        `INSERT INTO purchase_items
           (purchase_id, product_id, hsn, size, description, rate, quantity, discount, total)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [id, item.productId, item.hsn, item.size, item.description,
         item.rate, item.qty, item.discount || 0, item.total]
      );
      await conn.query(
        `UPDATE products SET stock = stock + ? WHERE id = ? AND shop_id = ?`,
        [item.qty, item.productId, req.shop.shop_id]
