...
router.get("/", async (req: AuthRequest, res) => {
  try {
    ...
    const [returns] = await pool.execute(
      `SELECT sr.*, s.invoice_no as original_invoice_no, s.created_at as original_sale_date
...
router.get("/sale/:saleId", async (req: AuthRequest, res) => {
  try {
    const { saleId } = req.params;
    const { shop_id } = req.shop!;
    const [returns] = await pool.execute(
      `SELECT sr.*, sri.product_id, sri.quantity, sri.rate, sri.total,
...
router.get("/available-items/:saleId", async (req: AuthRequest, res) => {
  try {
    const { saleId } = req.params;
    const { shop_id } = req.shop!;

    const [saleCheck] = await pool.execute(
    ...
    const [columns] = await pool.query(`SHOW COLUMNS FROM sale_items`);
    const columnNames = (columns as any[]).map((c) => c.Field);
    let qtyCol = "quantity";
    if (columnNames.includes("qty")) qtyCol = "qty";
    else if (columnNames.includes("sale_quantity")) qtyCol = "sale_quantity";
    ...
    const [items] = await pool.execute(query, [saleId]);

    if (!Array.isArray(items)) {
      return res.status(500).json({ error: "Unexpected DB result" });
    }

    const availableItems = (items as any[]).map((item) => {
...
router.post("/", async (req: AuthRequest, res) => {
  try {
    ...
    const saleCustomerRaw =
      sale.customer_id ?? sale.account_id ?? sale.party_id ?? sale.client_id ?? null;
    ...
    for (const item of items) {
      ...
      const [saleItems] = await pool.execute(
        `SELECT ${qtyCol} as qty FROM sale_items WHERE sale_id = ? AND product_id = ?`,
      ...
      const soldQty = Number((saleItems[0] as any).qty);
      const [returnedRows] = await pool.execute(
        `SELECT COALESCE(SUM(sri.quantity), 0) AS returned_qty
         FROM sales_return_items sri
         JOIN sales_returns sr ON sri.return_id = sr.id
         WHERE sri.product_id = ? AND sr.original_sale_id = ? AND sr.return_status = 'PROCESSED'`,
        [item.product_id, original_sale_id]
      );
      const returnedQty = Number((returnedRows as any[])[0]?.returned_qty || 0);
    ...
    try {
      ...
      await connection.execute(
        `INSERT INTO sales_returns 
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'PROCESSED')`,
        [returnId, original_sale_id, customer_id, shop_id, return_reason || null, total_amount, refund_method || "CREDIT", notes || null]
      );
      ...
      for (const item of items) {
        ...
        await connection.execute(
          `INSERT INTO sales_return_items 
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [itemId, returnId, item.product_id, item.quantity, item.rate, itemTotal, item.return_reason || "Customer return"]
        );
        ...
        await connection.execute(
          `INSERT INTO stock_history (id, product_id, shop_id, transaction_type, quantity, reference_id, notes, created_at)
           VALUES (?, ?, ?, 'SALES_RETURN', ?, ?, ?, NOW())`,
          [uuidv4(), item.product_id, shop_id, item.quantity, returnId, return_reason || "Sales return"]
        );
      }

      // 5. Accounting transaction — detect actual columns to avoid mismatch
      try {
        const [txColRows] = await pool.query(`SHOW COLUMNS FROM transactions`);
        const txCols = (txColRows as any[]).map((c) => c.Field);
        ...
        if (txCols.includes("amount") && txCols.includes("taxable_amount") && txCols.includes("tax_amount")) {
          ...
          await connection.execute(
            `INSERT INTO transactions 
             ...
             VALUES (?, NOW(), 'SALES_RETURN', ?, ?, ?, 0, ?, ?, ?)`,
            [transactionId, customer_id, total_amount, total_amount, return_reason || "Sales Return", original_sale_id, shop_id]
          );
          ...
          await connection.execute(
            `INSERT INTO transactions 
             ...
             VALUES (?, NOW(), 'SALES_RETURN', ?, ?, 0, ?, ?, ?)`,
            [transactionId, customer_id, total_amount, return_reason || "Sales Return", original_sale_id, shop_id]
          );
