...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const [existing] = await conn.query('SELECT id FROM eway_bills WHERE sale_id = ? AND status = "ACTIVE"', [sale_id]);
        if (existing.length > 0) {
            return res.status(400).json({ error: 'E-way bill already exists for this sale' });
        }
        // Get sale details
        const [saleResult] = await conn.query('SELECT * FROM sales WHERE id = ? AND shop_id = ?', [sale_id, req.shop.shop_id]);
        ...
        const [result] = await conn.query(`INSERT INTO eway_bills 
...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        const [tableCheck] = await conn.query(`SELECT COUNT(*) as count FROM information_schema.tables 
       WHERE table_schema = DATABASE() AND table_name = 'eway_bills'`);
        ...
        const [rows] = await conn.query(`SELECT eb.*, s.invoice_no, a.name as customer_name, s.total_amount, s.created_at as sale_date
       FROM eway_bills eb
       LEFT JOIN sales s ON eb.sale_id = s.id
       LEFT JOIN accounts a ON s.customer_id = a.id
       WHERE eb.shop_id = ?
       ORDER BY eb.generated_date DESC`, [req.shop.shop_id]);
...
router.get('/:id', async (req, res) => {
    ...
    try {
       ...
       LEFT JOIN sales s ON eb.sale_id = s.id
       LEFT JOIN accounts a ON s.customer_id = a.id
       WHERE eb.id = ? AND eb.shop_id = ?`, [req.params.id, req.shop.shop_id]);
        if (rows.length === 0) {
...
router.put('/:id/cancel', async (req, res) => {
    const conn = await db_1.default.getConnection();
    try {
        const [result] = await conn.query('UPDATE eway_bills SET status = "CANCELLED", updated_at = NOW() WHERE id = ? AND shop_id = ?', [req.params.id, req.shop.shop_id]);
