...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        const [shopRows] = await conn.query(`SELECT gstin, pincode FROM shops WHERE id = ?`, [req.shop.shop_id]);
...
router.post("/:id/payment", async (req, res) => {
    ...
    try {
        ...
        const [saleRows] = await conn.query(`SELECT * FROM sales WHERE id = ? AND shop_id = ?`, [id, req.shop.shop_id]);
        if (!saleRows.length)
        ...
        await conn.query(`UPDATE sales SET paid_amount = ?, status = ? WHERE id = ? AND shop_id = ?`, [newPaid, newStatus, id, req.shop.shop_id]);
        await conn.query(`INSERT INTO sale_payments (sale_id, shop_id, amount, payment_mode, note)
       VALUES (?, ?, ?, ?, ?)`, [id, req.shop.shop_id, amount, paymentMode, note || "Payment received"]);
        await conn.query(`UPDATE accounts SET balance = balance - ? WHERE id = ? AND shop_id = ?`, [amount, sale.customer_id, req.shop.shop_id]);
        await conn.commit();
...
router.get("/barcode/:invoiceNo", async (req, res) => {
    try {
        ...
        const [rows] = await db_1.default.query(`SELECT s.*, a.name AS customer_name, a.address AS customer_address,
       ...
       WHERE s.invoice_no = ? AND s.shop_id = ?`, [invoiceNo, req.shop.shop_id]);
        if (!rows.length)
            return res.status(404).json({ error: `Invoice not found: ${invoiceNo}` });
        const [items] = await db_1.default.query(`SELECT * FROM sale_items WHERE sale_id = ?`, [rows[0].id]);
...
router.put("/:id/tax-details", async (req, res) => {
    ...
    try {
        ...
        const [saleRows] = await conn.query(`SELECT s.*, a.gstin AS customer_gstin, a.pincode AS account_pincode
       FROM sales s
       ...
       WHERE id = ? AND shop_id = ?`, [
        ...
        const [updatedRows] = await conn.query(`SELECT s.*, a.name AS customer_name, a.address AS customer_address,
              a.phone AS customer_phone, a.gstin AS customer_gstin,
              COALESCE(s.customer_pincode, a.pincode) AS customer_pincode
...
router.get("/:id/download", async (req, res) => {
    try {
        const { id } = req.params;
        const [saleRows] = await db_1.default.query(`SELECT s.*, a.name AS customer_name, a.address AS customer_address,
              a.phone AS customer_phone, a.gstin AS customer_gstin,
              COALESCE(s.customer_pincode, a.pincode) AS customer_pincode
       FROM sales s JOIN accounts a ON a.id = s.customer_id
       WHERE s.id = ? AND s.shop_id = ?`, [id, req.shop.shop_id]);
        ...
        if (sale.through_agent)
            doc.text(`Through:  ${sale.through_agent}`, 50, startY + 85);
        ...
        items.forEach((item, i) => {
            if (i % 2 === 0)
                doc.rect(40, y, 515, rowH).fill("#fafafa").stroke();
            else
...
router.put("/:id", async (req, res) => {
    ...
    try {
        ...
        await conn.query(`UPDATE sales SET customer_id=?, total_qty=?, total_amount=?, paid_amount=?,
       ...
       WHERE id=? AND shop_id=?`, [
        ...
        for (const item of items) {
            await conn.query(`INSERT INTO sale_items (sale_id, product_id, hsn, size, description, rate, quantity, discount, total)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [id, item.productId, item.hsn, item.size, item.description, item.rate, item.qty, item.discount || 0, item.total]);
            await conn.query(`UPDATE products SET stock = stock - ? WHERE id = ? AND shop_id = ?`, [item.qty, item.productId, req.shop.shop_id]);
        }
        await conn.commit();
